...
/**
 * POST /api/escritorio/:id/desvincular
 * Desvincula un dispositivo de escritorio de forma segura usando el token de solicitud
 */
export async function desvincularEscritorioLocal(req, res) {
    try {
        const { id } = req.params;
        ...
        if (!token) {
            return res.status(400).json({
                success: false,
                message: 'El token de solicitud es requerido'
            });
        }

        // Desactivar el escritorio validando id, es_activo, token de solicitud, estado de la solicitud y MAC coincidente
        const resultado = await pool.query(`
            UPDATE escritorio e
            SET es_activo = false
            FROM solicitudes s
            WHERE e.id = $1 
              AND e.es_activo = true 
              AND s.token = $2 
              AND s.estado = 'aceptado' 
              AND s.mac = e.mac
            RETURNING e.id, e.nombre, e.empresa_id;
        `, [id, token]);

        if (resultado.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'No se pudo desvincular el dispositivo. Verifique el ID y el token.'
            });
        }

        const escritorio = resultado.rows[0];

        // Registrar evento
        await registrarEvento({
            titulo: 'Dispositivo de escritorio desvinculado localmente',
            descripcion: `El dispositivo "${escritorio.nombre}" fue desvinculado mediante atajo de teclado local`,
            tipo_evento: TIPOS_EVENTO.DISPOSITIVO,
            prioridad: PRIORIDADES.ALTA,
            detalles: {
                escritorio_id: id,
        ...
        console.error('Error en desvincularEscritorioLocal:', error);
        res.status(500).json({
            success: false,
